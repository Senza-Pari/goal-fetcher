 ...
 */
export const useNHLAPI = (): UseNHLAPIReturn => {
  ...
  const fetchData = async (endpoint: string, apiType: keyof typeof NHL_APIS = 'web'): Promise<any> => {
    ...
    for (let i = 0; i < CORS_PROXIES.length; i++) {
      ...
      try {
        ...
        toast({
          title: "NHL Data Retrieved!",
          description: `Successfully fetched data from ${apiType.toUpperCase()} API`,
        });
    ...
    toast({
      title: "NHL API Failed",
      description: "Could not retrieve data. Please try again later.",
      variant: "destructive",
    });
